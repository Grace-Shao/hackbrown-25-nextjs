...
# Scale the data
scaled_data = scaler.transform(unique_tracks.drop(columns=['album_name', 'track_name', 'artists']))
...
def recommend_songs(track_name):
    ...
    if track_name not in unique_tracks['track_name'].values:
        ...
        recommendations = []
        for i in indices[0]:
            recommendations.append((unique_tracks.iloc[i]['track_name'], unique_tracks.iloc[i]['artists']))
