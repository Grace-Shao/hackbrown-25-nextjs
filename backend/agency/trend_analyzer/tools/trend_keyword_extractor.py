...
class TrendKeywordExtractor(BaseTool):
    ...
    search_results: str = Field(
        ..., description="The search results text to extract keywords from"
    )
    max_keywords: int = Field(
        default=15, description="Maximum number of keywords to extract"
    )
    min_frequency: int = Field(
        default=2,
        description="Minimum frequency for a keyword to be considered trending",
    )
    ...
    def run(self):
        ...
        important_words = [
            word for word, tag in pos_tags if tag.startswith(("NN", "JJ", "VB"))
        ]
...
if __name__ == "__main__":
    sample_text = """
    The latest trends in music show a surge in viral hits and emerging artists.
    Music creators are increasingly focusing on social media platforms and streaming services.
    Several trending topics include viral music, emerging artists, and social media influence.
    The music industry is seeing a shift towards independent artists gaining popularity.
    """
    tool = TrendKeywordExtractor(
        search_results=sample_text, max_keywords=10, min_frequency=2
    )
    print(tool.run())
